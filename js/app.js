...
// gjør show tilgjengelig for inline/andre funksjoner
window.show = show;
...
const SUPABASE_ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx6dGl0amF1bXZxZ3ljcGN2dndsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjIyNjQ2NTEsImV4cCI6MjA3Nzg0MDY1MX0.ZDK2ymr5vYG3HP5-j-CRv3UAe5G-VKGIUOObbhd6cyU';
...
async function searchBrreg() {
  ...
  if (n.length !== 9) {
    brregLog.textContent = 'Org.nr må ha 9 siffer';
    return;
  }
  ...
  try {
    ...
    brregLog.textContent = 'Ikke funnet';
  }
}

// også tilgjengelig globalt ved behov
...
function attachRowLogic(row) {
  ...
  function maybeSetDefault() {
    const v = Number(dagerEl.value);
    if (!dagerEl.value || [356, 310, 104].includes(v)) {
...
function updateTariffDefault() {
  $$('#tariff-rows').innerHTML = '';
  addRow();
}

function resetForm() {
  [
    'orgnr', 'navn', 'besoksadresse', 'besoks_postnr',
    'besoks_poststed', 'firma', 'hoyttalere', 'kommentar'
  ].forEach(id => $$('#' + id).value = '');

  $$('#tariff-rows').innerHTML = '';
  brregLog.textContent = '';
  addRow();
...
async function saveControl(status) {
  ...
  if (!user) {
    toast('Innlogging kreves');
    return;
  }
  const payload = getFormData();
  const { error } = await supabase
    .from('controls')
    .insert({ user_id: user.id, orgnr: payload.orgnr || null, data: payload, status });
...
function buildPdf(doc, data) {
  ...
  doc.setFontSize(14);
  doc.text('TONO • GRAMO', 36, y, { align: 'center' });
  y += 7;

  doc.setFontSize(11);
  doc.text('Kontrollskjema for bruk av musikk', 36, y, { align: 'center' });
  y += 8;
  ...
  (data.tariff || []).forEach((t, i) => {
    const sit = (t.venue === 'Serveringssted' && t.seter)
      ? ` • ${t.seter} seter` : '';
    const mus = t.music === 'Bakgrunn'
      ? 'Bakgrunnsmusikk' : 'Fremtredende musikk';
    doc.text(
      `${i + 1}. ${t.venue} • ${mus} • ${t.kvm} kvm${sit} • ${t.dager} dager/år`,
      6,
      y
    );
    y += 5;
  ...
  y += 3;
  doc.text(`Høyttalere: ${data.hoyttalere || ''}`, 4, y);
  y += 5;
  ...
  y += 6;
  doc.text('Signert: ______________________', 4, y);
  y += 6;

  const d = new Date();
  doc.text(
    `Kontrollert: ${d.toLocaleDateString('nb-NO')} kl ${
...
${data.orgnr}
${data.tariff.map(t =>
  `${t.venue}/${t.music}: ${t.kvm}kvm/${t.dager}d`
).join('\n')}
${data.kommentar}`;

  const win = window.open('', '', 'width=360,height=520');
  win.document.write(
    '<pre style="font:14px monospace">' + sanitize(text) + '</pre>'
  );
  win.print();
}
...
async function openFromList(listId, index) {
  try {
    ...
    if (n.length === 9) {
      try {
        const r = await fetch(
          'https://data.brreg.no/enhetsregisteret/api/enheter/' + n
        );
        if (r.ok) {
          const d = await r.json();
          const a = d.forretningsadresse || d.beliggenhetsadresse || {};
...
async function renderDashboard() {
  ...
  lists.forEach(L => {
    ...
    L.items.forEach((it, i) => {
      const hay = [
        it.Orgnummer, it.Navn, it.Gateadresse,
        it.Postnr, it.Poststed, it.Kommentar
      ].join(' ').toLowerCase();
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn-tiny"
                  title="Åpne i skjema"
                  onclick="openFromList('${L.id}', ${i})">
            ${sanitize(it.Navn || 'Åpne')}
          </button>
        </td>
...
async function renderArchive() {
  ...
  const byMonth = {};
  rows.forEach(c => {
    const m = new Date(c.created_at)
      .toLocaleDateString('nb-NO', { year: 'numeric', month: 'long' });
...
$$('#magicBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (error) throw error;
    toast('Sjekk e-post (magisk lenke)');
  } catch (e) {
    showAuthError(e);
    toast('Kunne ikke sende lenke');
  }
});
...
$$('#sendBtn').addEventListener('click', () => {
  const payload = getFormData();
  generatePDF(payload);
  saveControl('sent').catch(console.error);
});
